...
using webvideoplayback::server::Socket;
using webvideoplayback::server::WinsockGuard;
using webvideoplayback::server::create_listener;
using webvideoplayback::server::send_all;
...
std::optional<ByteRange> parse_range(const std::string& header, std::uint64_t file_size)
{
    ...
    if (start_text.empty()) {
        const std::optional<std::uint64_t> suffix = webvideoplayback::utils::parse_u64(end_text);
        if (!suffix || *suffix == 0) {
            return std::nullopt;
        }
    ...
    const std::optional<std::uint64_t> start = webvideoplayback::utils::parse_u64(start_text);
    ...
    if (!end_text.empty()) {
        const std::optional<std::uint64_t> parsed_end = webvideoplayback::utils::parse_u64(end_text);
...
std::filesystem::path resolve_request_path(const std::filesystem::path& root, std::string target)
{
    ...
    target = webvideoplayback::utils::url_decode(target);
...
std::filesystem::path resolve_request_path(const ServerConfig& config, std::string target)
{
    ...
    target = normalize_route(webvideoplayback::utils::url_decode(target));
...
{
    const std::string extension = webvideoplayback::utils::lower_copy(path.extension().string());
...
std::optional<Request> read_request(SOCKET socket)
{
    ...
    while (std::getline(stream, line)) {
        ...
        const std::string name = webvideoplayback::utils::lower_copy(webvideoplayback::utils::trim(std::string_view(line).substr(0, colon)));
        const std::string value = webvideoplayback::utils::trim(std::string_view(line).substr(colon + 1));
        if (name == "range") {
            request.range = value;
        }
    }
...
ServerConfig parse_args(int argc, char** argv)
{
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (arg == "--config" && index + 1 < argc) {
            ...
            const std::optional<std::uint64_t> port = webvideoplayback::utils::parse_u64(argv[++index]);
